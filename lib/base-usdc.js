...
async function verifyBaseUsdcTransfer(expected, options = {}) {
  ...
  const verified = verifyReceiptTransfer(receipt, { ...expected, transactionHash: txHash });

  const blockNumberHex = String(receipt && receipt.blockNumber || "").toLowerCase();
  if (!/^0x[a-f0-9]+$/.test(blockNumberHex)) {
    throw rpcError("BASE_BLOCK_NUMBER_REQUIRED", "Verified receipt is missing a valid Base block number.");
  }

  const block = await rpcCall("eth_getBlockByNumber", [blockNumberHex, false], options);
  const timestampHex = String(block && block.timestamp || "").toLowerCase();

  if (!/^0x[a-f0-9]+$/.test(timestampHex)) {
    throw rpcError("BASE_BLOCK_TIMESTAMP_REQUIRED", "Verified Base block is missing a valid timestamp.");
  }

  const blockTimestampSeconds = Number.parseInt(timestampHex.slice(2), 16);

  if (!Number.isSafeInteger(blockTimestampSeconds) || blockTimestampSeconds <= 0) {
    throw rpcError("BASE_BLOCK_TIMESTAMP_INVALID", "Verified Base block timestamp is invalid.");
  }

  return {
    ...verified,
    block_timestamp: new Date(blockTimestampSeconds * 1000).toISOString()
  };
}
